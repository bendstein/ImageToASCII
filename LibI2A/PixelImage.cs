...
public class PixelImage
{
    ...
    public List<double> GetIntensities((double H, double S, double V)? HSVWeights = null) => Pixels.Select(p => p?.GetIntensity(HSVWeights ?? (1, 1, 1)) ?? 0).ToList();
    ...
    public IEnumerable<PixelImage> Tiles(int width, int height)
    {
        ...
        var padded_image_width = Width + width - (Width % width == 0 ? width : Width % width);
        var padded_image_height = Height + height - (Height % height == 0 ? height : Height % height);
        ...
        for(var k = 0; k < padded_image_height;)
        {
            List<List<IMagickColor<ushort>?>> row = [];

            for(var j = 0; j < height; j++, k++)
            {
                for(var i = 0; i < padded_image_width; i++)
                {
