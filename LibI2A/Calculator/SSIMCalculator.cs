...
public class SSIMCalculator : ISSIMCalculator
{
    ...
    public double Calculate(PixelImage image_a, PixelImage image_b)
    {
        ...
        var subtile_width_a = (int)Math.Ceiling(image_a.Width / Math.Pow(2, options.Subdivide));
        var subtile_height_a = (int)Math.Ceiling(image_a.Height / Math.Pow(2, options.Subdivide));
        var subtile_width_b = (int)Math.Ceiling(image_b.Width / Math.Pow(2, options.Subdivide));
        var subtile_height_b = (int)Math.Ceiling(image_b.Height / Math.Pow(2, options.Subdivide));
    ...
    private static double[] GenerateGaussian(int w, int h, double stddev)
    {
        ...
        for(var i = 0; i < w; i++)
        {
            for(var j = 0; j < h; j++)
            {
                var value = 1d / (2 * Math.PI * Math.Pow(stddev, 2))
